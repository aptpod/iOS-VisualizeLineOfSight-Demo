//
//  LeftVisualizeDegreesView.swift
...
class LeftVisualizeDegreesView: UIView {
    ...
    private func setupLayout() {
        ...
        let lineView = UIView.init(frame: CGRect.init(x: self.frame.width-lineWidth/2, y: (self.frame.height-lineWidth)/2, width: lineWidth, height: lineWidth))
        // Split Degrees
        let splitDegrees = 180 / DEGREES_SPLIT_VALUE
        for i in 0...DEGREES_SPLIT_VALUE {
            let view = UIView.init(frame: CGRect.init(x: (lineView.frame.width-lineWidth)/2, y: (lineView.frame.height-LINE_WIDTH)/2, width: lineWidth, height: LINE_WIDTH))
